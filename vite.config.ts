...
// Load site configuration if it exists, otherwise use defaults
let siteConfiguration: FigmaSiteConfiguration = {}
try {
  siteConfiguration = JSON.parse(readFileSync('./.figma/make/site.json', 'utf-8'))
} catch {
...
export default defineConfig(({ mode }) => {
  ...
  return {
    base: process.env.FIGMA_PUBLIC_URL ? `${process.env.FIGMA_PUBLIC_URL}/` : '/SP-Consultancy/',
    build: {
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "Sneha & Prahar Consultancy Services"
